...
pub(crate) fn run_background_post_tick(node: &mut crate::control::node::Node, state_dir: &Path) {
    ...
    let _ = crate::run_queue::network_bridge::process_pending_run_queue_results(node, state_dir);
    let _ =
        crate::run_queue::network_bridge::process_pending_stigmergy_contributions(node, state_dir);
    if !slow_post_tick_due(state_dir, Instant::now()) {
        return;
    }
    let _ =
        crate::run_queue::network_bridge::evaluate_open_stigmergy_rounds_for_state(node, state_dir);
