...
use wattswarm_storage_core::storage::{
    CsMailboxDeliveryState, LocalAgentEventRow, NetworkBackendStatusRow,
    PendingNetworkCommandInsert, PgStore, local_control_scope_id,
};
...
#[test]
fn postgres_transport_queues_preserve_lease_and_retry_semantics() {
    with_test_schema(|| {
        let store = PgStore::open("transport-queues.state").expect("open postgres store");
        ...
        assert!(
            store
                .complete_pending_network_command(
                    scope_id,
                    "command-1",
                    &final_claim[0].lease_token
                )
...
#[test]
fn postgres_migrates_legacy_pending_command_status_constraint() {
    with_test_schema(|| {
        let conn = open_test_connection();
        conn.execute_batch(
            "CREATE TABLE pending_network_commands_local (
             ...
             ) VALUES ('scope', 'legacy-command', 'test', '{}', 'queued',
                       clock_timestamp(), clock_timestamp());",
        )
        .expect("install legacy postgres command constraint");
        drop(conn);

        let store = PgStore::open("legacy-command-status.state").expect("migrate postgres schema");
        let claim = store
...
#[test]
fn postgres_migrates_legacy_client_server_transport_columns() {
    with_test_schema(|| {
        let conn = open_test_connection();
        conn.execute_batch(
            "CREATE TABLE cs_outbound_progress_local (
             ...
             ) VALUES ('scope','events','global_bulk',7,5,clock_timestamp());
             ...
             ) VALUES ('scope','gap-legacy','interactive',1,'{}','expired',
                       clock_timestamp(),clock_timestamp(),1,clock_timestamp());",
        )
        .expect("create legacy ClientServer tables");
        drop(conn);

        let store = PgStore::open("legacy-client-server.state")
            .expect("migrate legacy ClientServer tables");
        let progress = store
...
#[test]
fn postgres_client_server_progress_and_delivery_state_are_durable() {
    with_test_schema(|| {
        let store = PgStore::open("client-server-state.state").expect("open postgres store");
        ...
        drop(store);
        let store =
            PgStore::open("client-server-state-reopen.state").expect("reopen postgres store");
        let pending = store
            .list_cs_mailbox_pending_commits("scope")
